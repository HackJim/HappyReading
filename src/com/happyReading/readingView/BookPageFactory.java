...
public class BookPageFactory {
	...
	public BookPageFactory(int w, int h) {
		...
		mLineCount = (int) (mVisibleHeight / (m_fontSize + divideSize)); // 可显示的行数
	}
	...
	public void setBgBitmap(Bitmap BG, int width, int height) {
		m_book_bg = BG;
		Matrix matrix = new Matrix();
		float scaleX = ((float) width) / m_book_bg.getWidth();
		float scaleY = ((float) height) / m_book_bg.getHeight();
		matrix.postScale(scaleX, scaleY);
		Bitmap dstbmp = Bitmap.createBitmap(m_book_bg, 0, 0,
				m_book_bg.getWidth(), m_book_bg.getHeight(), matrix, true);
		m_book_bg = dstbmp;
	}

	public void setM_fontSize(int m_fontSize) {
		this.m_fontSize = m_fontSize;
		mLineCount = (int) (mVisibleHeight / (m_fontSize + divideSize));
	}
